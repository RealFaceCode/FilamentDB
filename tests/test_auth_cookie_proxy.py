...
class AuthCookieProxyTests(unittest.TestCase):
    ...
    def test_settings_cookie_without_tls_is_not_secure_by_default(self):
        client = TestClient(app, base_url="http://testserver")

        response = client.post(
            "/settings",
            data={"lang": "de", "next_url": "/dashboard"},
            follow_redirects=False,
        )

        self.assertEqual(response.status_code, 303)
        set_cookie = response.headers.get("set-cookie", "")
        self.assertIn("lang=de", set_cookie)
        self.assertNotIn("; Secure", set_cookie)

    def test_settings_cookie_with_forwarded_https_is_secure(self):
        client = TestClient(app, base_url="http://testserver")
